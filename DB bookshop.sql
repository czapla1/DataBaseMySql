...
(8, 'Mateusz', 'Skorupa','male', '1981-08-31', 'Gdańsk', '01-590', 'Nadmorska', '89', 'pomorskie', '712456644', 'mat_skor@wp.pl'),
...
# FOR ADMIN
#Showing customers who have all payments paid
SELECT 
    customers.id_customer, firstname, lastname, amount, payment_status, payment_date
FROM
    customers
        JOIN
    payments ON (customers.id_customer = payments.id_customer
        AND payment_status = 'zaplacono');

select * from payments;        


#Showing all payments including these not paid (where column payment_date is null) 
SELECT 
    id_customer,
    id_payment,
    payment_date,
...
select round(sum(amount), 2) 'Total paid in May' from payments where month(payment_date) =4;


#Showing title of books which was sent in April
select books.id_book,title from books join orders using (id_book) where month(order_date) =4 ;

#Showing 3 most expensive books
select id_book,title,price from books order by price desc limit 3;


#Showing how many books from each category are offered
select category, count(*) 'Qty per category' from books group by category order by 'Qty per category' ;


#Showing how many categories is offered
select count(distinct category) 'Qty of categories' from books;


#Showing name of offered categories
select distinct category as 'Offered categories' from books;
...
CREATE VIEW orders_open AS
    ...
    WHERE
        books.id_book = orders.id_book
            AND order_status = 'oczekiwanie';

select *from orders_open;


#Creating view presenting books from category Informatyka
CREATE VIEW books_it AS
    SELECT 
        id_book, title, category, description, 
		author_name, author_surname, price, ISBN, publisher,quantity, status_book, publishing_date
    FROM
        books
    WHERE
...
select *from cust_prov_mazowieckie;


#Creating trigger to keep tracking how many female buy books
select count(*) from customers where gender='female';

create table women(gender varchar(20), qty int);
insert into women(gender, qty) values ('female',4);

create trigger trig_women after insert on  customers for each row update women set qty=qty+1 where gender=new.gender;

insert into customers (id_customer, firstname, lastname, gender,
birthday_date,  city, post_code, street, home_number, province, 
phone_number, email) values  
(11, 'Agata', 'Wrobel','female', '1992-12-29', 'Warszawa', '04-490', 'Mlynarska', '178', 'mazowieckie', '744408198', 'aga_wro@wp.pl');

insert into login (id_customer, login, passwort) values  
(11, 'login11', 'passwort11');

select *from women;


#Creating trigger (insert) to keep tracking how many books from category Informatyka are in bookshop
select count(*) as total from books where category= 'Informatyka';

create table books_it_total(category varchar(50), total_qty int);
insert into books_it_total(category, total_qty) values ('Informatyka',6);
select * from books_it_total;

create trigger trig_books_total after insert on books for each row update books_it_total set total_qty=total_qty+1 where category=new.category;

insert into books (id_book, title, category, description, 
author_name, author_surname, price, ISBN, publisher,quantity,status_book, publishing_date) values
(13, 'Python dla dzieci','Informatyka','Ksiazka stanowi atrakcyjne wprowadzenie do swiata programowania','Jason','Briggs', 38.50,'9788301185633', 'PWN',72, 'produkt dostepny');

insert into books (id_book, title, category, description, 
author_name, author_surname, price, ISBN, publisher,quantity, status_book, publishing_date) values
(14, 'Lowca cieni','Kryminal','Mroczne sekrety wiecznego miasta','Donato','Carrsi', 27.99,'9788379859672', 'Albatros',55, 'produkt dostepny');

select * from books;
delete from books_it_total where total_qty=5;
delete from books where id_book>=13;


#Creating trigger (delete) to keep tracking how many books from category Informatyka are in bookshop
create trigger trig_books_total_del after delete on books for each row update books_it_total set total_qty=total_qty-1 where category=old.category;
...
#Creating view presenting 5 last orders for specified user
CREATE VIEW last_5_orders AS
    SELECT 
        orders.id_order,
        books.title,
        books.price,
...
select * from last_5_orders;


#Creating view presenting status of last 5 payments for specified user 
CREATE VIEW last_5_payments AS
    SELECT 
        orders.id_order, title, amount, payment_status
    FROM
        payments
            NATURAL JOIN
        orders
...
CREATE VIEW not_paid_payments AS
    ...
    FROM
        ...
        books ON (books.id_book = orders.id_book)
    WHERE
        payment_status= 'niezaplacono' and id_customer = 9;
...
CREATE VIEW available_books AS
    SELECT 
        title, description, 
		author_name, author_surname, price, ISBN, publisher,quantity, status_book
    FROM
        books
    WHERE
...
CREATE VIEW books_kulinaria AS
    SELECT 
        title, description, 
		author_name, author_surname, price, ISBN, publisher,quantity, status_book
    FROM
...
#Creating view presenting books with key word 'Java'
